...
[CustomPropertyDrawer(typeof(Vector2EventSo))]
public class Vector2EventPropertyDrawer : PropertyDrawer
{
    ...
    private void DrawSubAssetProperty(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            // Draw Vector2EventSo field without ability to edit
            GUI.enabled = false;
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(Vector2EventSo), property.serializedObject.targetObject);
            GUI.enabled = true;
            ...
            if (_isPropertyShown)
            {
                ...
                valueProperty.vector2Value = EditorGUI.Vector2Field(valuePosition, "", valueProperty.vector2Value);
        ...
        else
        {
            ...
            // Draw Vector2EventSo property and register value changes
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(Vector2EventSo), property.serializedObject.targetObject);

            // Draw create Button
            if (GUI.Button(createButtonRect, "Create"))
            {
                // Create new instance of the Vector2EventSo and set it
    ...
    /// <summary>
    /// Creates a new Vector2EventSo as a sub-asset.
    /// </summary>
    public Vector2EventSo CreateSubVariable(Object parentObject, string name)
    {
        // Creating Instance and renaming it to the property name
        var variableSO = ScriptableObject.CreateInstance<Vector2EventSo>();
