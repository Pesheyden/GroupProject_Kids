...
[CustomPropertyDrawer(typeof(ObjectEventSo))]
public class ObjectEventPropertyDrawer : PropertyDrawer
{
    ...
    private void DrawSubAssetProperty(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            // Draw ObjectEventSo field without ability to edit
            GUI.enabled = false;
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(ObjectEventSo), property.serializedObject.targetObject);
            GUI.enabled = true;
        ...
        else
        {
            ...
            // Draw ObjectEventSo property and register value changes
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(ObjectEventSo), property.serializedObject.targetObject);

            // Draw create Button
            if (GUI.Button(createButtonRect, "Create"))
            {
                // Create new instance of the ObjectEventSo and set it
    ...
    private void DrawStandardProperty(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            // Draw Value property and register value changes
            valueProperty.objectReferenceValue = EditorGUI.ObjectField(valuePosition, valueProperty.objectReferenceValue, typeof(Object));
    ...
    /// <summary>
    /// Creates a new ObjectEventSo as a sub-asset.
    /// </summary>
    public ObjectEventSo CreateSubVariable(Object parentObject, string name)
    {
        // Creating Instance and renaming it to the property name
        var variableSO = ScriptableObject.CreateInstance<ObjectEventSo>();
