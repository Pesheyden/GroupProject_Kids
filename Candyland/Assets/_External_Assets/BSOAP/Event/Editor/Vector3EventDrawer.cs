...
[CustomPropertyDrawer(typeof(Vector3EventSo))]
public class Vector3EventPropertyDrawer : PropertyDrawer
{
    ...
    private void DrawSubAssetProperty(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            // Draw Vector3EventSo field without ability to edit
            GUI.enabled = false;
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(Vector3EventSo), property.serializedObject.targetObject);
            GUI.enabled = true;
            ...
            if (_isPropertyShown)
            {
                ...
                valueProperty.vector3Value = EditorGUI.Vector3Field(valuePosition, "", valueProperty.vector3Value);
        ...
        else
        {
            ...
            // Draw Vector3EventSo property and register value changes
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(Vector3EventSo), property.serializedObject.targetObject);

            // Draw create Button
            if (GUI.Button(createButtonRect, "Create"))
            {
                // Create new instance of the Vector3EventSo and set it
    ...
    /// <summary>
    /// Creates a new Vector3EventSo as a sub-asset.
    /// </summary>
    public Vector3EventSo CreateSubVariable(Object parentObject, string name)
    {
        // Creating Instance and renaming it to the property name
        var variableSO = ScriptableObject.CreateInstance<Vector3EventSo>();
