...
[CustomPropertyDrawer(typeof(IntEventSo))]
public class IntEventDrawer : PropertyDrawer
{
    ...
    private void DrawSubAssetProperty(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            // Draw IntEventSo field without ability to edit
            GUI.enabled = false;
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(IntEventSo), property.serializedObject.targetObject);
            GUI.enabled = true;
        ...
        else
        {
            ...
            // Draw IntEventSo property and register value changes
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(IntEventSo), property.serializedObject.targetObject);

            // Draw create Button
            if (GUI.Button(createButtonRect, "Create"))
            {
                // Create new instance of the IntEventSo and set it
    ...
    /// <summary>
    /// Creates a new IntEventSo as a sub-asset.
    /// </summary>
    public IntEventSo CreateSubVariable(Object parentObject, string name)
    {
        // Creating Instance and renaming it to the property name
        var variableSO = ScriptableObject.CreateInstance<IntEventSo>();
