...
[CustomPropertyDrawer(typeof(FloatEventSo))]
public class FloatEventPropertyDrawer : PropertyDrawer
{
    ...
    private void DrawSubAssetProperty(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.objectReferenceValue != null)
        {
            ...
            // Draw FloatEventSo field without ability to edit
            GUI.enabled = false;
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(FloatEventSo), property.serializedObject.targetObject);
            GUI.enabled = true;
        ...
        else
        {
            ...
            // Draw FloatEventSo property and register value changes
            property.objectReferenceValue = EditorGUI.ObjectField(objectFieldRect, property.objectReferenceValue, typeof(FloatEventSo), property.serializedObject.targetObject);

            // Draw create Button
            if (GUI.Button(createButtonRect, "Create"))
            {
                // Create new instance of the FloatEventSo and set it
    ...
    /// <summary>
    /// Creates a new FloatEventSo as a sub-asset.
    /// </summary>
    public FloatEventSo CreateSubVariable(Object parentObject, string name)
    {
        // Creating Instance and renaming it to the property name
        var variableSO = ScriptableObject.CreateInstance<FloatEventSo>();
